<template>
  <div id="waifu" style="position: absolute;top: 50px;">
    ...
    <div id="waifu-tool"><live2dItem :class-name="className" @itemClick="itemClick" @itemEnter="itemEnter" /></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loadWidget(config) {
      ...
      let userActionTimer;
      const messageArray = ['好久不见，日子过得好快呢……', '大坏蛋！你都多久没理人家了呀，嘤嘤嘤～', '嗨～快来逗我玩吧！', '拿小拳拳锤你胸口！', '记得把小家加入 Adblock 白名单哦！'];
    ...
    welcomeMessage() {
      ...
      if (now > 5 && now <= 7) this.text = '早上好！一日之计在于晨，美好的一天就要开始了。';
      else if (this.now > 7 && this.now <= 11) this.text = '上午好！工作顺利嘛，不要久坐，多起来走动走动哦！';
      else if (this.now > 11 && this.now <= 13) this.text = '中午了，工作了一个上午，现在是午餐时间！';
      else if (this.now > 13 && this.now <= 17) this.text = '午后很容易犯困呢，今天的运动目标完成了吗？';
      else if (this.now > 17 && this.now <= 19) this.text = '傍晚了！窗外夕阳的景色很美丽呢，最美不过夕阳红～';
      else if (this.now > 19 && this.now <= 21) this.text = '晚上好，今天过得怎么样？';
      else if (this.now > 21 && this.now <= 23) this.text = ['已经这么晚了呀，早点休息吧，晚安～', '深夜时要爱护眼睛呀！'];
